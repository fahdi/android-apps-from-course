...
(function($) {
	...
	var _endDate;
	var calendarEvents;
	var calendarEventsArray = new Array();
	var prevent
	var defaults = {
			height: 650,
			width: 980,
			...
			onCellBlockOver: function(event) { return true; },
			onCellBlockOut: function(event) { return true; },
	...
	jQuery.J.DrawCalendar = function(dateIn, current_parent_container){
		...
		var prevMLink = jQuery('<div class="MonthNavPrev"><a href="" class="link-prev" tabindex="99">'+ defaults.navLinks.p +'</a></div>').click(function() {
			var current_container = jQuery(this).parents('div[class^="jMonthCalendar"]');
			jQuery.J.ChangeMonth(prevMonth, current_container);
			return false;
		});
		...
		var nextMLink = jQuery('<div class="MonthNavNext"><a href="" class="link-next" tabindex="99">'+ defaults.navLinks.n +'</a></div>').click(function() {
			var current_container = jQuery(this).parents('div[class^="jMonthCalendar"]');
			jQuery.J.ChangeMonth(nextMonth, current_container);
			return false;
		});
		...
		if(defaults.navLinks.enablePrevYear) {
			prevYLink = jQuery('<div class="YearNavPrev"><a href="" tabindex="99">'+ prevYear.getFullYear() +'</a></div>').click(function() {
				var current_container = jQuery(this).parents('div[class^="jMonthCalendar"]');
				jQuery.J.ChangeMonth(prevYear, current_container);
				return false;
			});
		}
		...
		if(defaults.navLinks.enableNextYear) {
			nextYLink = jQuery('<div class="YearNavNext"><a href="" tabindex="99">'+ nextYear.getFullYear() +'</a></div>').click(function() {
				var current_container = jQuery(this).parents('div[class^="jMonthCalendar"]');
				jQuery.J.ChangeMonth(nextYear, current_container);
				return false;
			});
		}
		...
		if(defaults.navLinks.enableToday) {
			...
			todayLink = jQuery('<div class="TodayLink"></div>').click(function() {
				var current_container = jQuery(this).parents('div[class^="jMonthCalendar"]');
				jQuery.J.ChangeMonth(new Date(), current_container);
				return false;
			});
		}

		//Build up the Header first,  Navigation
		var navRow = jQuery('<tr><td colspan="7"><div class="FormHeader MonthNavigation"></div></td></tr>').css({ "height" : 0 });
		...
		monthNavHead.append(jQuery('<div class="TodayLink"></div>').append(defaults.locale.months[d.getMonth()] + " " + d.getFullYear()));
		...
		headRow = jQuery("<thead id=\"CalendarHead\"></thead>").append(headRow).css('width', defaults.width);
		...
		do {
			...
			for (var i=0; i<7; i++) {
				...
				//DateBox Events
				var dateLink = jQuery('<div class="DateLabel"><a tabindex="' + _currentDate.getDate()*10 + '" '+ (isToday ? 'title="today"' : '') +' >' + _currentDate.getDate() + '</a></div>').click(function(e) {
                    defaults.onDayLinkClick(new Date($(this).parent().attr("date")));
                    e.stopPropagation();
                });
				
				var event_holder_link = jQuery("<div class='event-outer-holder'></div>");
				
				var dateBox = jQuery("<td></td>").attr(atts).append(dateLink).append(event_holder_link).dblclick(function(e) {
		...
		if (current_parent_container) {
			var a = current_parent_container.css({ "width" : defaults.width + "px" });
			var current_parent_holder = current_parent_container;
		} else {
			var a = jQuery(defaults.containerId).css({ "width" : defaults.width + "px", "height" : defaults.height + "px" });
			var current_parent_holder = jQuery(defaults.containerId);
		}
		...
		var parentEventContainerId = current_parent_holder.attr('id');
		if( typeof calendarEventsArray[parentEventContainerId + '_events_list'] != "undefined" ) 
			var current_events = calendarEventsArray[parentEventContainerId + '_events_list'];
		else
			var current_events = calendarEvents;
		
		DrawEventsOnCalendar(current_events, current_parent_holder);
	}
	
	var DrawEventsOnCalendar = function(current_events, current_parent_holder) {
		
		if (current_events)
			 calendarEvents = current_events;
		 
		if (calendarEvents && calendarEvents.length > 0) {
			var headHeight = defaults.labelHeight + defaults.navHeight;
			var dtLabelHeight = jQuery(".DateLabel:first", defaults.containerId).outerHeight();
			
			if (current_parent_holder)
				var parent_container_box = current_parent_holder;
			else
				var parent_container_box = jQuery(defaults.containerId);
			jQuery.each(calendarEvents, function(){
				...
				if(sDt) {
					if ((sDt >= _beginDate) && (sDt <= _endDate)) {	
						...
						var cell = jQuery("#" + getDateId(sDt), parent_container_box);
						var label = jQuery(".DateLabel", cell);
						label.attr('id', "event_container_"+ ev.EventID);
						var holder = jQuery(".event-outer-holder", cell);
						holder.attr('id', "event_holder_"+ ev.EventID);
						var tabindex = Number(jQuery('a',label).attr('tabindex'));
						if(jQuery.browser.msie && jQuery.browser.version == '8.0'){
							label.after('<span tabindex="'+ (tabindex + 1) +'" class="event-data">' + ev.Title + '</span>');
						}
						var event  = holder.append('<div class="Event" id="Event_' + ev.EventID + '"><span tabindex="'+ (tabindex + 1) +'" class="event-data">' + ev.Title + '</span><span tabindex="'+ (tabindex + 2) +'" class="event-description">' + ev.Description + '</span><a tabindex="'+ (tabindex + 3) +'" href="' + ev.URL + '">&rsaquo; Full Details</a></div>');
						var link = jQuery('<a tabindex="'+ (tabindex + 4) +'" href="' + ev.URL + '">&rsaquo; Full Details</a>');
						...
						if (!cell.hasClass('Inactive')) {
							cell.tipTip({content: get_tooltip_content(ev.EventID), keepAlive:true, maxWidth: "200px"});
						} else {
							event.remove();
						}
					}
			...
			jQuery('a',parent_container_box).focus(function(){
				cell = $(this).parents('td.DateBox');
				parent_container_box.find('td.hasEvent').not('#'+cell.attr('id')).trigger('mouseout');
				$(this).parents('td').trigger('mouseover');
			});
		}
	}
	...
	jQuery.J.ChangeMonth = function(dateIn, current_container) {
		defaults.onMonthChanging(dateIn);
		jQuery.J.DrawCalendar(dateIn, current_container);
	...
	jQuery.J.Initialize = function(options, events) {
		...
		var event_calendar_container_id = options.containerId;
		event_calendar_container_id = event_calendar_container_id.replace("#", "") + '_events_list'; 
		calendarEventsArray[event_calendar_container_id] = events;
		if(events)
		{
